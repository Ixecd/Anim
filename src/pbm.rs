...
/// 防御激活层级——Anim 在 Pass 6 中执行的信号安全缩放强度。
///
/// 不是"创伤分类器"。是"防御性缩放器"。
/// 触发条件来自 VSA PBM 向量的相变标记——不是用户自述的任何叙事标签。
///
/// 特性：
///   - 只有 PBM 内部状态机能上调此层级。外部输入（含用户显式反馈）不可直接写入。
///   - 叙事污染检测在防御激活之前执行——若信号呈叙事化特征（重复自我指涉、
///     高抽象归因、跨 Session 叙事一致性），直接冻结该维权重——不放大。
///
/// 分级：
///   - D1：禁主不禁点缀（安全类原子除外）——强度上限不受影响
///   - D2：禁主 + 点缀配比减半 + 强度上限同等缩减
///   - D3：全禁 + LeakRate→~0 + CriticalThreshold→极低 + P0 抢占就绪 + 主动麻痹
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
