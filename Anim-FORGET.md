...
2026-06-01  v0.1.13 深度架构审计——12个设计盲区
            - animi 不是编译器——是交织器（interlinker）。本文件及所有文档，编译→交织。
            - #1  Session中用户状态突变安全盲区——PSIR只在启动时校验一次，运行中cap降到30但旧参数继续输出。需10Hz轻量安全看门狗。
            - #2  混合原子包强度叠加绕过沙箱——核心原子+沙箱原子混合时总强度可能超上限。沙箱强度必须独立校验。
            - #3  创伤用户点缀配比的绝对强度——禁主不禁点+配比≤0.10不考虑绝对强度×创伤敏感系数。需创伤敏感系数+绝对阈值。
            - #4  后台预编译FSIR缓存失效缺失——全局安全规则更新后本地缓存FSIR仍含旧参数。FSIR需携带安全规则版本号+Registry哈希。
            - #5  设备热插拔安全重校验缺失——设备断开重连后DSIR不重做安全校验。任何热插拔→触发DSIR级安全重校验。
            - #6  oi帧缺失无平滑处理——oi跳过帧=abrupt_stop=强度>20需知情同意。oi应输出线性插值过渡帧而非直接跳过。
            - #7  异常终止Session数据污染——安全插桩强制终止后PBM要不要更新。需数据置信度分级——异常Session只更新安全阈值不更新基线。
            - #8  新用户预测器冷启动灾难——无历史数据→预测准确率<50%→频繁安全插桩→数据进一步污染。前10次Session强制关闭预测器。
            - #9  跨维度阻尼矩阵无具体参数——只有概念没有规则。情绪梯度>X冻结内脏,Y冻结触觉，回落到Z解冻。需制定具体参数表。
            - #10 宏递归组合风险绕过——组合风险计算是在展开前还是展开后。必须在完全展开后的AST上计算组合风险。
            - #11 运行期oi可观测性为零——硬件bit不携带上下文，日志只有"oi触发"。FPGA需oi原因寄存器（4位编码16种原因）。
            - #12 outline模式安全约束缺失——缺设备时降级行为未定义。每个原子需声明设备依赖等级+降级策略。
