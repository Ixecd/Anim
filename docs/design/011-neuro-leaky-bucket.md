...
本地闭环自适应校准 ——
    ...
    NeuroEnergyTracker 在 v0.4 实现时——直接订阅穿戴设备总线暴露的物理生理指标——
    HRV 骤降、皮电异常攀升、深睡时长缩短——
    当检测到系统未触发熔断——但用户的生理指标已显示"受体通胀/耗竭"特征时——
    由本地运行时控制器在设备端——自动调低 CriticalThreshold 或调高 M/N 恢复配比。
    不是等云端——是在设备本地——用自己的 PBM 历史——自己修正自己的参数。
```
