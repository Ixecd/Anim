...
原则：能在 Pass 2 拒绝的，不留到 Pass 3。能在交织期插桩的，不等到运行期。运行期的神经信号已经进入人体了——安全校验必须在信号生成之前完成。
...
Anim 不是人类的专用语言。感受不只人有——万物皆有。Anim 的泛型让同一份 `.anim` 源码可以被参数化为不同物种的交织目标。
