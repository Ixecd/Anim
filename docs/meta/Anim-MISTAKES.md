...
## 设计决策澄清——不是错误，是值得记住的"为什么"

### D01: Rust 用 4 空格而非 tab 缩进

- **问题**: 为什么四个字符不用一个 tab？1 个 tab 比 4 个空格少 75% 的存储。
- **答案**: 缩进用 tab 没问题——对齐必须用空格。一旦混用——不同编辑器 tab 宽度设置不同（2/4/8）——代码对齐直接裂开。Rust 的模式匹配、链式调用、泛型约束——对齐无处不在。Go 能用 tab 是因为 `gofmt` 处理了混用的对齐层——Go 语法够简单——对齐场景少。Rust 做不到——对齐太密集——统一用空格是最安全的选择。不是偏好问题——是跨环境确定性——牺牲 75% 压缩率换 100% 视觉一致性。
- **日期**: 2026-06-01
