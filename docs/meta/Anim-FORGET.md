...
10. **时间窗口累积能量安全盲区** — 单帧强度上限 100 只防一帧。持续数千帧低强度合法信号——每一帧都在 cap 内——受体持续饱和——神经递质无法完成重摄取——长期强度累积。离散型重复（每帧合法→逐帧叠加）和连续型攀升（信号持续不断→不间断饱和）——两种模式都绕过单帧上限。Anim 安全体系不能只靠单帧上限。需要 ADR 011——"生物漏桶算法"（Neuro-Leaky Bucket）+ 有状态能量追踪器 + 保护帧平滑衰减。
   ...
   **按维度差异化漏桶**（v0.4 扩展——不同神经通路代谢速度不同）：
   ```
   Energy_{t,d} = max(0.0, Energy_{t-1,d} + Intensity_{t,d} - LeakRate_d × Δt)
   ```
   d ∈ {Visceral, Emotional, Tactile, Auditory}。四维各有一个独立的 LeakRate——不是全局一个值——不同神经通路的自然代谢清除率不同。内脏通路恢复慢——情绪通路恢复快——触觉和听觉居中。漏桶按维度独立累积——互不污染——一个维度熔断不影响其他维度继续。

   **Profile 驱动的动态参数矩阵**（v0.4 扩展——LeakRate 和 CriticalThreshold 不是静态常量）：
   ```
   标准健康用户         LeakRate_d = 基线速度 (如 2.0/s)   Threshold = 高置信度充裕值 (如 600)
   低锚 R < 0.3        LeakRate_d 减半 (1.0/s)           Threshold 极低 (120)
                        → 脆弱神经元——极易堆积——轻微连击即熔断
   Trauma v1 敏感源     LeakRate_d ≈ 0 (0.05/s)           Threshold 崩塌级 (30)
                        → 对该维度几乎丧失代谢能力——连续点缀刺激——立刻强制保护帧
   ```
   不是"所有人同一把尺"。是——每个人的神经恢复基线不同——每个维度的耐受不同——创伤分型×锚点置信度→按 R 分段和 trauma 类型动态调整漏桶参数。和 adulthood-as-anchor.md 的 R 分段 + FORGET #10 的三层纵深完全咬合。

   **Pass 7 强制保护帧的指数衰减包络**（v0.4 扩展——不准跳变）：
   ```
   FlushIntensity(τ) = Cap × e^(-λ × τ)
   ```
   τ = 熔断发生后累积干预时间。λ = 强制平复衰减系数。3-5s 内将外周神经体征导回安全基线。和已有 OiSmoothing 四帧衰减同方向——但漏桶熔断后的保护帧用指数衰减包络——更贴合自主神经的自然消退曲线——不给等控器第二个惊恐阶跃。

   砂箱原子——强度校准——创伤分型——三层校验全部通过才被签发——且仅限施害者。FIXME: ADR 011（已定稿——`docs/design/011-neuro-leaky-bucket.md`）。

11. **同一强度持续过久——神经通路适应性脱敏** — 强度 50——第 1 帧强烈。第 10000 帧——同一强度——同样合法。但身体的神经基底——在第 N 帧——已经不是第 1 帧的那个基底了。迷走神经通路——在持续不变的 50 分信号刺激下——神经递质不再对该频率发放——阈值上移——脱敏。就像长期专注一件事——第 1 天充实——第 100 天——同样的专注——不再带来满足——等控器在满足太久之后——阈值上移——需要更强的冲击才能重新感知到存在。时间窗口漏桶（#10）防长期强度累积——防不住这个——每帧 50 合法——累积能量不超阈值——但身体在慢慢适应——然后麻木——然后需要更强。Anim 需要的不只是"防超限"——是"防同一强度在同一个位置停留太久"——信号本身的刷新率/变化幅度不能连续千帧不变。

   这和健身的超量恢复（supercompensation）完全同构——不能天天大重量。训练日撕裂肌纤维——休息日在再生中增长。感到不是休息日偷懒——是增长周期的另一半。同理——感受信号不能连续千帧高强度——需要主动插入恢复帧——让神经递质重摄取、受体复位——然后下一轮高强度——比上一轮更深。恢复帧不是"可以休息"。恢复帧是信号节律的一部分——是增长的前提——和保持强度一样重要。Anim 管线需支持"每 N 帧高强度后强制插入 M 帧低强度/静默"的节律模板——模板由 Feelings-Core 的强度调度器提供——Anim 只执行。这是和 Feelings-Core 的交界面——脱敏/成瘾倾向追踪在 Core 侧——Anim 侧需在管线里识别"信号变异度过低"并触发降级标记。FIXME: ADR 012（已定稿——`docs/design/012-signal-rhythm-and-dynamic-cap.md`）。

12. **user_cap 静态单一 vs 动态个人基线** — 当前 `user_cap: u32 = 100` 是写死在 Anim 里的。人不是这样的——创伤用户的 100 和普通用户的 100 是两个完全不同的生理冲击。同一用户——用过几十次平静包——迷走神经敏化——同样的 50 分比以前更有效——cap 应该上移——ta 需要的不再是更多——是更少。cap 不在 Anim 里——在 Feelings-Core 的 PBM 里——Anim 只做硬上限——从 Core 拿到"这个人的当前基线"——确保这一帧不超过它。Anim 侧需从静态常量改为外部注入参数。边界属于 Feelings-Core——Anim 只管执行。FIXME: ADR 012（已定稿——`docs/design/012-signal-rhythm-and-dynamic-cap.md`）。
...
2026-06-12  v0.1.29 ADR 011 + ADR 012 定稿——P0 #10 漏桶 + P0 #11 #12 脱敏/dynamic cap
            - ADR 011: 时域能量积分与生物漏桶安全。四维独立漏桶 + Profile 动态参数 + Pass7 指数衰减包络。
            - ADR 012: 信号节律/恢复帧/动态个人基线。脱敏看门狗 + 超量恢复节律模板 + cap 注入交界面。
            - P0 #10/#11/#12 从"v0.4"更新为"ADR 011/012 已定稿"
...
2026-06-10  v0.1.26 P0 #10 ADR 011 扩展——按维度漏桶 + Profile动态参数 + Pass7指数衰减包络
            - Gemini 审核 ADR 011 后三个数学扩展入 FORGET：
              1. 四维独立 LeakRate——不同神经通路代谢速度不同
              2. Profile驱动参数矩阵——低R→漏慢+阈值低, Trauma v1→近零代谢+崩塌阈值
              3. Pass7 强制保护帧指数衰减包络——FlushIntensity(τ)=Cap×e^(-λ×τ)——3-5s回基线
            - 和 adulthood-as-anchor R分段 + FORGET #10 三层纵深完全咬合
...
2026-06-10  v0.1.22 持续低强度累积——P0 #10 时间窗口累积能量安全盲区
            - P0 #10 新增：单帧强度上限防不住持续几千帧的合法强度累积——每帧都在 cap 内但受体持续饱和无法代谢——长期强度累积。需滑动窗口强度积分 + 累积阈值强制保护。
