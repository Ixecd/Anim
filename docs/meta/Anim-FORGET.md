...
10. **时间窗口累积能量安全盲区** — 单帧强度上限 100 只防一帧。持续数千帧低强度合法信号——每一帧都在 cap 内——受体持续饱和——神经递质无法完成重摄取——累积致命。离散型重复（每帧合法→逐帧叠加）和连续型攀升（信号持续不断→不间断饱和）——两种模式都绕过单帧上限。Anim 安全体系不能只靠单帧上限。需要 ADR 011——"生物漏桶算法"（Neuro-Leaky Bucket）+ 有状态能量追踪器 + 保护帧平滑衰减。
...
11. **同一强度持续过久——神经通路适应性脱敏** — 强度 50——第 1 帧强烈。第 10000 帧——同一强度——同样合法。但身体的神经基底——在第 N 帧——已经不是第 1 帧的那个基底了。迷走神经通路——在持续不变的 50 分信号刺激下——神经递质不再对该频率发放——阈值上移——脱敏。就像长期专注一件事——第 1 天充实——第 100 天——同样的专注——不再带来满足——等控器在满足太久之后——阈值上移——需要更强的冲击才能重新感知到存在。时间窗口漏桶（#10）防累积致命——防不住这个——每帧 50 合法——累积能量不超阈值——但身体在慢慢适应——然后麻木——然后需要更强。Anim 需要的不只是"防超限"——是"防同一强度在同一个位置停留太久"——信号本身的刷新率/变化幅度不能连续千帧不变。
...
2026-06-10  v0.1.22 持续低强度累积——P0 #10 时间窗口累积能量安全盲区
            - P0 #10 新增：单帧强度上限防不住持续几千帧的合法强度累积——每帧都在 cap 内但受体持续饱和无法代谢——累积致命。需滑动窗口强度积分 + 累积阈值强制保护。
