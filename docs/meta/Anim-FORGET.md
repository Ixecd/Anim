...
27. **无版本兼容性检查** — FSIR 无 `fsir_version` 字段，不同版本 animi 无法判断兼容性。
...
### D01: 包名与感受原子解耦
- 包名（feeling name）≠ 主旋律原子（main atom）。包名是给创作者的人类标签——主旋律是给岛叶的信号。
- 同一个包名下可以有多种主旋律——"冷静"不是一个东西——是一族状态。
- `calm_meditative` / `gentle_focus` / `deep_rest` = 三种不同的 calm。由创作者选择。
- Anim 不替创作者决定——只保证类型正确+安全。
