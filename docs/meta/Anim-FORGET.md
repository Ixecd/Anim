...
10. **时间窗口累积能量安全盲区** — 单帧强度上限 100 只防一帧。凌迟 3000 刀——每一刀强度远低于上限——但持续几千帧——身体被推到崩溃。明朝洗刷——开水浇→铁刷刷——同样——每一帧不超限——累积致命。Anim 安全体系不能只靠单帧上限。需在时间窗口上做强度积分（如 window_energy = Σ intensity_i, 过去 30s/5min）——积分超过阈值→触发强制保护帧。Jitter Buffer 不是这个——Jitter Buffer 是对齐时钟——这是对"每个合法帧的合法强度在时间轴上的累积致命性"——做保护。FIXME: ADR 011。
