...
11. **同一强度持续过久——神经通路适应性脱敏** — 强度 50——第 1 帧合法。第 10000 帧——同一强度——同样合法。但身体的神经基底——在第 N 帧——已经不是第 1 帧的那个基底了。迷走神经通路——在持续不变的 50 分信号刺激下——神经递质不再对该频率发放——阈值上移——脱敏。然后同样的 50 分——不平静了。需要更高强度——通胀。时间窗口漏桶（#10）防累积致命——防不住这个——每帧 50 合法——累积能量不超阈值——但身体在慢慢适应——然后麻木——然后需要更强。Anim 需要的不只是"防超限"——是"防同一强度在同一个位置停留太久"——信号本身的刷新率/变化幅度不能连续千帧不变。这是和 Feelings-Core 的交界面——脱敏/成瘾倾向追踪在 Core 侧——Anim 侧需在管线里识别"信号变异度过低"并触发降级标记。FIXME: v0.4。
...
```
2026-06-10  v0.1.24 P0 #11 + #12——脱敏与动态基线 cap
            - P0 #11 新增：同一强度持续过久→神经通路适应性脱敏→通胀。防累积≠防脱敏。需信号刷新率/变化幅度检测。
