...
10. **时间窗口累积能量安全盲区** — 单帧强度上限 100 只防一帧。凌迟 3000 刀——每一刀远低于上限——持续几千帧——身体被推崩溃。洗刷——开水浇→铁刷刷——每一帧不超限——累积致命。剥皮楦草——不是一刀一刀的离散信号——是从第一刀切入到整片皮肤剥离——连续几千帧的皮肤剥离信号——每一帧强度可被控制在 cap 内——但神经末梢在持续暴露中——累积的感受强度在时间窗口上远超单帧上限能表达的任何限制。Anim 安全体系不能只靠单帧上限。需在时间窗口上做强度积分（如 window_energy = Σ intensity_i, 滑动窗 30s/5min）——积分超过阈值→触发强制保护帧。Jitter Buffer 不是这个——Jitter Buffer 是对齐时钟——这是对"每个合法帧的合法强度在时间轴上的累积致命性"——做保护。砂箱原子——强度校准——创伤分型——三层校验——全部通过才被签发——且仅限施害者。FIXME: ADR 011。
